...
pub const InterruptibleCommandMagic = struct {
    ...
    command: [*:0]const u8 = "true",
    pid: std.os.pid_t = 0,
    item: *c.GtkCheckMenuItem = undefined,
    ...
    fn waitForCompletion(self: *Self) void {
        const fd = @intCast(std.os.fd_t, @truncate(u32, std.os.linux.pidfd_open(self.pid, 0)));
        defer _ = std.os.linux.close(fd);
        std.event.Loop.instance.?.waitUntilFdReadable(fd);
        std.debug.print("Command `{s}` finished\n", .{self.command});

        c.gtk_check_menu_item_set_active(self.item, 0);
    }
    ...
    fn start(self: *Self) void {
        ...
        if (pid == 0) {
            ...
            self.pid = pid;
            std.event.Loop.instance.?.runDetached(self.allocator, Self.waitForCompletion, .{self}) 
                catch return self.stop();
        }
    }
    ...
    pub fn addToMenu(self: *Self, menu: *c.GtkMenu) void {
        ...
        self.item = @ptrCast(*c.GtkCheckMenuItem, item);
    }
};
